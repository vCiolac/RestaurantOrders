...
def get_sorted_dishes_from_menu(menu: List[DictDish]) -> List[DictDish]:
    return sorted(menu, key=lambda dish: dish["dish_name"])
...
def test_get_main_menu_returns_a_data_frame():
    ...
    assert all(isinstance(dish, dict) for dish in menu)
...
def test_get_main_menu_data_frame_have_correct_keys():
    ...
    menu_keys = set()
    for dish in menu:
        menu_keys.update({*dish.keys()})

    assert {"dish_name", "ingredients", "price", "restrictions"} == menu_keys
...
def test_get_main_menu_returns_all_dishes_when_called_with_no_restrictions(
    ...
    given_dishes = get_sorted_dishes_from_menu(menu)
