...
@pytest.mark.dependency(name="req_5_1_part_3")
def test_check_recipe_availability_not_okay_with_more_than_one_ingredient():
    ...
    assert not is_recipe_available, (
        f"Recipe with {(ingredient_1, amount_1) = } and"
        f"{(ingredient_1, amount_1) = } should not be available"
    )
